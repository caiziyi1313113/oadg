...
class MaxIoUAssigner:
    ...
    def assign(self, bboxes, gt_bboxes, gt_bboxes_ignore=None, gt_labels=None):
        ...
        if bboxes.shape[0] == 0 or gt_bboxes.shape[0] == 0:
            # No gt or no candidates: assign all as background.
            num_bboxes = bboxes.shape[0]
            assigned_gt_inds = jt.zeros((num_bboxes,), dtype=jt.int32)
            max_overlaps = jt.zeros((num_bboxes,), dtype=jt.float32)
            assigned_labels = None
            if gt_labels is not None:
                assigned_labels = jt.zeros((num_bboxes,), dtype=jt.int32)
            return AssignResult(0, assigned_gt_inds, max_overlaps, labels=assigned_labels)
