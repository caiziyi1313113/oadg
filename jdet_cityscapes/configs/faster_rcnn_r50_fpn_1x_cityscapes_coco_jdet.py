...
# 训练输出目录和数据集目录
name = "faster_rcnn_r50_fpn_cityscapes_coco_jdet"
if os.name == "nt":
    work_dir = r"D:\sim2real\OA-DG\jdet_cityscapes\work_dirs\faster_rcnn_r50_fpn_cityscapes_coco_jdet"
    cityscapes_root = r"D:\sim2real\OA-DG\ws\data\cityscapes"
else:
    work_dir = "/mnt/d/sim2real/OA-DG/jdet_cityscapes/work_dirs/faster_rcnn_r50_fpn_cityscapes_coco_jdet"
...
model = dict(
    ...
    # bbox_head：二阶段分类+回归
    bbox_head=dict(
        type='SharedFCBBoxHeadHBB',
        num_fcs=2,
        in_channels=256,
        fc_out_channels=1024,
        roi_feat_size=7,
        num_classes=num_classes,# 这里根据实际情况
        target_means=[0.0, 0.0, 0.0, 0.0],
